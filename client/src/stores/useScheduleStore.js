...
export const useScheduleStore = create((set, get) => ({
  ...
  updateScheduleOptimistic: async (schedule) => {
    ...
    try {
      ...
      set({
        schedules: schedulesArray.map((s) => (s.id === schedule.id ? schedule : s))
      })
  ...
  updateSchedule: async (schedule) => {
    ...
    try {
      ...
      const savedSchedule = await scheduleService.save(schedule)
      set((state) => ({
        schedules: Array.isArray(state.schedules) ? state.schedules.map((s) => (s.id === savedSchedule.id ? savedSchedule : s)) : [savedSchedule]
      }))
      return savedSchedule
    } catch (err) {
