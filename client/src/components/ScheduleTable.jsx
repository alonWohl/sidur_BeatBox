...
export const ScheduleTable = React.memo(
  ({
    ...
    // Reset selectedEmployee when schedule changes
    useEffect(() => {
              ...
              <TableRow key={`${shift}-${position}`} className="transition-colors">
                ...
                {DAYS.map((day) => (
                  <TableCell
                    ...
                    className={`p-0 w-[80px] border ${isToday(day) ? 'bg-blue-50/40' : ''} ${highlightedDay === day ? 'bg-yellow-50' : ''} 
              ...
              <TableRow key={`${role}-${position}`} className="h-10 transition-colors hover:bg-gray-50/30">
                ...
                {DAYS.map((day) => (
                  <TableCell
                    ...
                    className={`border p-0 transition-colors ${isToday(day) ? 'bg-blue-50/40' : ''} ${highlightedDay === day ? 'bg-yellow-50' : ''} 
