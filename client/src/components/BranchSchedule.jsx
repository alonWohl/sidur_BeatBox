		...
		return Array.from({length: roleConfig.positions}, (_, index) => {
			...
			return (
				<TableRow key={`${role}-${position}`}>
					<TableCell
						className={`text-center font-medium bg-gray-50 border-l text-sm sm:text-base sticky right-0 z-10 ${
					...
					{DAYS.map((day) => (
						<TableCell
							...
							className={`${position === 1 ? '' : 'border-t-0'}`}>
			...
			<div className='flex flex-col p-1 container mx-auto'>
				...
				<div
					...
					<Table
						...
						<TableHeader>
							<TableRow>
								<th className='text-center text-base sticky right-0 z-10 bg-gray-50 w-28 border border-gray-200 p-2'>
									תפקיד
								</th>
                    ...
                    `}
										...
										<div
											className={`text-sm mt-1 ${
												isToday ? 'text-[#BE202E] font-medium' : 'text-gray-600'
											}`}>
											{date}
										</div>
									</th>
						...
						<TableBody>
							...
							<TableRow>
								<TableCell
									...
									className='h-0.5 p-0 bg-gray-300'
								/>
							</TableRow>
